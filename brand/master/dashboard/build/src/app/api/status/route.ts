...
export async function GET(request: NextRequest) {
  try {
    ...
    const envKeys = [
      'NEXT_PUBLIC_BRAND_SUPABASE_URL',
      'NEXT_PUBLIC_BRAND_SUPABASE_ANON_KEY',
    ...
    // Check Supabase Configuration (Master brand-prefixed)
    const supabaseUrl = process.env.NEXT_PUBLIC_BRAND_SUPABASE_URL
    const anonKey = process.env.NEXT_PUBLIC_BRAND_SUPABASE_ANON_KEY
    ...
    if (!status.supabaseConfig.urlValid) {
      status.recommendations.push('NEXT_PUBLIC_BRAND_SUPABASE_URL is missing or invalid. Check your .env.local file.')
    }
    if (!status.supabaseConfig.anonKeyValid) {
      status.recommendations.push('NEXT_PUBLIC_BRAND_SUPABASE_ANON_KEY is missing or invalid. Get it from Supabase Dashboard > Settings > API.')
    }
