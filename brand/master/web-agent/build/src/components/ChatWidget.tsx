...
const ICONS = {
  ...
  ai: (brand: string, config?: BrandConfig) => {
    ...
    // Fallback: Use image logo for master, infinity symbol for others
    if (brand === 'master' && config && config.chatStructure?.avatar?.source) {
      return <img src={config.chatStructure.avatar.source} alt={config.name} style={{ width: '100%', height: '100%' }} />;
    }
...
export function ChatWidget({ apiUrl, widgetStyle = 'searchbar' }: ChatWidgetProps) {
  ...
  const brandKey: 'master' = 'master';
  const finalApiUrl = apiUrl || config.apiUrl || '/api/master/chat';
    ...
    const initializeSession = async () => {
      try {
        if (process.env.NODE_ENV !== 'production') {
          console.log('[ChatWidget Master] Initialising session', { brandKey });
        }
        ...
        // Initialize session in Supabase immediately when chat opens (master only)
        if (brandKey === 'master') {
          try {
            const { initializeSessionOnOpen } = await import('@/lib/chatSessions');
            const sessionRecord = await initializeSessionOnOpen(storedId, 'master');
            if (sessionRecord) {
    ...
    if (isOpen && messages.length === 0 && !hasRestoredMessagesRef.current && !hasShownWelcomeRef.current && externalSessionId) {
      ...
      const fetchConversationsOnReopen = async () => {
        try {
          ...
          if (addAIMessage) {
            addAIMessage("Hi! I'm here to help you understand Master brand, ask me anything.");
            hasShownWelcomeRef.current = true;
          }
        } catch (err) {
          console.error('[ChatWidget] Error fetching conversations on reopen:', err);
      ...
      addAIMessage("Hi! I'm here to help you understand Aviation training at {{BRAND_NAME}}, ask me anything.");
      hasShownWelcomeRef.current = true;
    }
