...
// Build user profile progressively for Master
...
export async function updateMasterProfile(
  externalSessionId: string,
  profileData: Partial<MasterUserProfile>,
  brand: 'master' = 'master'
): Promise<void> {
  const supabase = getSupabaseClient();
  if (!supabase) {
    console.warn('[chatSessions] Supabase client unavailable in updateMasterProfile', { brand });
    return;
  }
  ...
  if (!currentSession) {
    console.error('[updateMasterProfile] Session not found', { externalSessionId });
    return;
  }
  ...
  if (Object.keys(updates).length > 0) {
    ...
    if (error) {
      console.error('[updateMasterProfile] Failed to update session', error);
    }
  }
  ...
  if (phone) {
    ...
    if (leadId) {
      // Update all_leads.unified_context.master
