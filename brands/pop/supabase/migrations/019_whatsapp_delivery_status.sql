...
-- NOTE: original master migration had `AND created_at > NOW() - INTERVAL '7 days'`
-- here, which Postgres rejects (NOW() is not IMMUTABLE, illegal in an index
-- predicate). Dropped the time clause; the cron filters created_at at query time.
