...
type Trace = {
  redisCacheHit: boolean; redisReadMs: number | null; provider: string | null
  appApiMs: number | null; providerStartMs: number | null; vobizDialMs: number | null
  groqMs: number | null; groqCacheHit: boolean | null; notes?: string[]
}
type Call = {
...
function TraceChip({ label, value, tone, hint }: { label: string; value: string; tone?: string; hint?: string }) {
  return (
    <span title={hint} style={{
      display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 9px', borderRadius: 8,
      background: 'var(--bg-primary)', border: '1px solid var(--border-primary)', color: tone || 'var(--text-secondary)',
      fontSize: 10.5, fontWeight: 800, whiteSpace: 'nowrap',
    }}>
      <span style={{ color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.35 }}>{label}</span>
      <span style={{ fontVariantNumeric: 'tabular-nums' }}>{value}</span>
    </span>
  )
}

function TraceStrip({ trace }: { trace?: Trace | null }) {
  if (!trace) return null
  const msVal = (v: number | null | undefined) => (v == null ? '—' : `${Math.round(v)} ms`)
  return (
    <div style={{ margin: '2px 0 14px' }}>
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 0.5, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 7 }}>
        Full path trace
      </div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <TraceChip label="Redis" value={trace.redisCacheHit ? `hit · ${msVal(trace.redisReadMs)}` : `miss · ${msVal(trace.redisReadMs)}`} tone={trace.redisCacheHit ? '#22c55e' : '#eab308'} hint="Redis stores short-lived voice telemetry snapshots used by this bench." />
        <TraceChip label="App API" value={msVal(trace.appApiMs)} hint="Time spent in our outbound test-call API before the provider accepted/rejected the start." />
        <TraceChip label="VoBiz/SIP" value={msVal(trace.vobizDialMs)} hint="Dial/start time through the telephony trunk or V3 pipeline proxy when measured." />
        <TraceChip label="Provider start" value={msVal(trace.providerStartMs)} hint="Provider or pipeline start request latency measured by our server." />
        <TraceChip label="Groq" value={trace.groqMs == null ? 'not in path' : `${msVal(trace.groqMs)}${trace.groqCacheHit ? ' · cache' : ''}`} tone={trace.groqMs == null ? 'var(--text-muted)' : '#22c55e'} hint="Groq only appears when the live voice path calls our Groq-backed agent core. Vapi/ElevenLabs hosted agents do not expose it here." />
      </div>
      {trace.notes && trace.notes.length > 0 && (
        <div style={{ marginTop: 7, color: 'var(--text-muted)', fontSize: 11.5, lineHeight: 1.45 }}>
          {trace.notes[0]}
        </div>
      )}
    </div>
...
function EngineBadge({ engine }: { engine: 'vapi' | 'elevenlabs' | 'sarvam' }) {
  const is2 = engine === 'elevenlabs'
  const is3 = engine === 'sarvam'
  const tone = is3 ? '#8b5cf6' : is2 ? '#f59e0b' : '#14b8a6'
  return (
...
function SplitRow({ engine, split }: { engine: 'vapi' | 'elevenlabs' | 'sarvam'; split: EngineSplit }) {
...
function V3Row() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end', opacity: 0.72 }}>
      ...
      <span title="Sarvam pipeline is wired; place a V3 call to record it here" style={{ fontSize: 10.5, fontWeight: 700, color: '#8b5cf6', background: '#8b5cf618', border: '1px solid #8b5cf644', borderRadius: 999, padding: '4px 10px', cursor: 'help' }}>Sarvam · ready</span>
    </div>
  )
}
...
export default function CallsView() {
  ...
  const view = useMemo(() => {
    ...
    const splitFor = (engine: 'vapi' | 'elevenlabs' | 'sarvam'): EngineSplit | null => {
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', borderTop: '1px solid var(--border-primary)' }}>
      ...
      <div style={{ flexShrink: 0, padding: '12px 18px 10px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7, alignItems: 'flex-start' }}>
          ...
          {view.sarvam ? <SplitRow engine="sarvam" split={view.sarvam} /> : (eng === 'all' || eng === 'sarvam') && <V3Row />}
          ...
          {eng !== 'sarvam' && !view.vapi && !view.elevenlabs && !view.sarvam && shown.length > 0 && (
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '0 18px 18px' }}>
        ...
        {!loading && configured && shown.length === 0 && !err && (
          <div style={{ padding: 24, borderRadius: 12, background: 'var(--bg-secondary)', border: '1px solid var(--border-primary)', color: 'var(--text-secondary)', fontSize: 13 }}>
            {eng === 'sarvam'
              ? 'No V3 calls recorded yet. Place a V3 test call from the Voice agent tab, then refresh this bench.'
