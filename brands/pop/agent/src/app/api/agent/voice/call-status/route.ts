...
// Normalizes an Indian phone number the same way the test-call routes do.
...
// V2/V3 have no end-of-call webhook (unlike Vapi), so this poller is the only
// place that ever learns a call ended — run the same enrichment Vapi gets
// (transcript/summary row + lead scoring) here, once, when `ended` flips true.
// Guarded on the session's own call_status so repeated polls don't re-trigger it.
async function enrichIfNewlyEnded(
  ...
  try {
    const { data: sess } = await supabase
      .from('voice_sessions')
      .select('call_status, customer_phone, customer_phone_normalized, call_direction')
    ...
    await enrichCompletedCall({
      ...
      phone,
      phoneNorm,
      direction: sess.call_direction === 'inbound' ? 'inbound' : 'outbound',
...
// test-call/route.ts stamps a `call_summary: 'engine:<name>'` marker on the
// voice_sessions row for V2/V3 (there's none for V1/Vapi) — use that as the
// source of truth when a session row exists, falling back to the query param
// only when there's no row yet (e.g. status polled before the insert lands).
