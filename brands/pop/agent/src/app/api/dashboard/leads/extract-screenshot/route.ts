...
// ~7MB of base64 (Anthropic caps source images at ~5MB decoded).
const MAX_BASE64_LEN = 7_000_000

const SYSTEM_PROMPT = `You read a screenshot of a WhatsApp (or similar messaging) chat / lead form and pull out business-lead details for BCON, an AI-first marketing agency that sells to businesses.
  ...
  try {
    ...
    try {
      raw = await generateFromImage(
        SYSTEM_PROMPT,
        'Extract the lead details from this chat/form screenshot as JSON.',
        base64,
        mediaType,
      )
    } catch (err: any) {
    ...
    // Defensive: strip accidental ```json fences and pull the first {...} block.
