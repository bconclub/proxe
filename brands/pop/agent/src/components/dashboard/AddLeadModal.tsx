...
export default function AddLeadModal({ isOpen, onClose, onCreated }: AddLeadModalProps) {
  ...
  const runExtraction = useCallback(async (dataUrl: string) => {
    ...
    try {
      ...
      const found = ['name', 'phone', 'email', 'city', 'business_name', 'business_type', 'service_interest']
        .filter((k) => ex[k]).length
      setExtractMsg(
        found > 0
  ...
  }, [name, phone, email, city, businessName, businessType, serviceInterest, websiteStatus, leadVolume, urgency, note, resetForm, onCreated, onClose])
                ...
                >
                  ...
                  {imagePreview ? (
                    ...
                    <div className="flex flex-col items-center gap-1.5 py-2">
                      ...
                      <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>Drop a chat / form screenshot</span>
                      <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>or click / paste — we&rsquo;ll read name, number &amp; business details</span>
                    </div>
                  )}
                </div>
