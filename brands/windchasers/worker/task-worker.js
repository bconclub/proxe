/**
 * Windchasers Autonomous Task Worker - Sequence Engine
 *
...
const WA_TEMPLATE_NAME = process.env.WA_TEMPLATE_NAME || 'windchasers_followup';
...
const VOBIZ_ANSWER_URL = process.env.VOBIZ_ANSWER_URL || 'https://proxe.windchasers.in/api/agent/voice/answer';
...
const ANGLE_MESSAGES = {
  ...
  urgency: (name) =>
    `${name}, we've got a few spots open for a free Demo Class this week. Come see the training facility and ask everything. Want me to lock one in?`,
 ...
 * Aria introduces Windchasers for the first time. No prior contact assumed.
 */
...
async function sendDailyReport() {
  ...
  try {
    ...
    const body =
      `<b>Windchasers Daily Report - ${dateStr}</b>\n\n` +
...
async function pollTelegramCommands() {
  ...
  for (const update of updates) {
    ...
    if (text === '/status') {
      ...
      const { data: activeLeads } = await supabase.from('all_leads').select('id').in('brand', ['windchasers', 'default']).not('lead_stage', 'in', '("Converted","Closed Won","Closed Lost","Cold")');

      await sendTelegram(chatId,
        `<b>Windchasers Status</b>\n\n` +
...
async function sendWeeklyReport() {
  ...
  try {
    ...
    const body =
      `<b>Windchasers Weekly Intelligence</b>\n\n` +
      `<b>Performance:</b>\n` +
...
async function executeTask(task) {
  ...
  if (TEMPLATE_TASK_TYPES.includes(task.task_type) && task.lead_id) {
    ...
    const FOLLOWUP_TEMPLATES = ['windchasers_proxe_followup_engaged', 'windchasers_proxe_followup_noengage'];
    const threeDaysAgo = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString();
  ...
  switch (task.task_type) {
    ...
    case 'booking_reminder_24h': {
      ...
      return await executeSendMessage(task, waPhone,
        `${task.lead_name}, your call with the Windchasers team is tomorrow at ${fmt24}. See you there.`);
    }
    ...
    case 'reminder_24h': {
      ...
      const result = await executeSendMessage(task, waPhone,
        `Hey ${task.lead_name}! Just a reminder - you have a call with Windchasers tomorrow at ${task.metadata?.booking_time}. Looking forward to connecting!`);
    ...
    case 'reminder_1h': {
      const sessionId = task.metadata?.session_id;
      const result = await executeSendMessage(task, waPhone,
        `Hi ${task.lead_name}! Your call with Windchasers is in about an hour at ${task.metadata?.booking_time}. Looking forward to it!`);
    ...
    case 'reminder_30m': {
      const sessionId = task.metadata?.session_id;
      const result = await executeSendMessage(task, waPhone,
        `${task.lead_name}, your Windchasers call is in 30 minutes at ${task.metadata?.booking_time}. See you soon!`);
    ...
    case 'follow_up_day5':
      return await executeSequenceStep(task, waPhone,
        `${task.lead_name}, haven't heard back. Still interested in becoming a pilot? Last chance to book your free Demo Class.`);
    case 're_engage':
    ...
    case 'post_booking_confirmation':
      return await executeSendMessage(task, waPhone,
        `Great news ${task.lead_name}! Your Demo Class with Windchasers is confirmed for ${task.metadata?.booking_date} at ${task.metadata?.booking_time}. We're looking forward to meeting you. See you then!`);
...
async function executePushToBook(task, waPhone) {
  ...
  return await executeSendMessage(task, waPhone,
    `Hey ${task.lead_name}! We had a great chat earlier. Would love to set up a free Demo Class at Windchasers so you can see the training facility and ask everything. When works this week?`);
}
...
function extractServiceFromContext(ctx) {
  ...
  const topicMap = [
    ...
    { keywords: ['demo class', 'demo flight', 'trial class', 'demo'], label: 'a Demo Class' },
...
const TEMPLATE_PARAM_COUNT = {
  'windchasers_proxe_booking_reminder_24h': 3,  // name, time, service
  'windchasers_proxe_booking_reminder_30m': 3,  // name, service, time
  'windchasers_proxe_reengagement_engaged': 2,  // name, pain_point
  'windchasers_proxe_reengagement_noengage': 1, // name
  'windchasers_proxe_first_outreach': 1,        // name
  'windchasers_proxe_post_call_followup': 1,    // name
  'windchasers_proxe_followup_engaged': 2,      // name, service
  'windchasers_proxe_followup_noengage': 2,     // name, service
  'windchasers_proxe_rnr': 1,                   // name
};

// Template quick reply button labels matching Meta-approved templates
const TEMPLATE_BUTTONS = {
  'windchasers_proxe_followup_engaged': ['Yes, let\'s go'],
  'windchasers_proxe_followup_noengage': ['Yes, tell me more', 'Just exploring'],
  'windchasers_proxe_booking_reminder_24h': ['Yes, I\'ll be there', 'No, I need to reschedule'],
  'windchasers_proxe_booking_reminder_30m': ['I\'m ready!'],
  'windchasers_proxe_reengagement_engaged': ['Yes, let\'s talk'],
  'windchasers_proxe_reengagement_noengage': ['Yes Lets Talk'],
};

// Template body texts matching Meta-approved templates (used to render human-readable content for conversation logs)
const TEMPLATE_BODIES = {
  'windchasers_proxe_booking_reminder_24h': `Hi {{customer_name}}, your call with the Windchasers team is tomorrow at {{booking_time}}.\nWe'll be covering {{service_interest}}.\nSee you there.`,
  'windchasers_proxe_booking_reminder_30m': `Hi {{customer_name}}, 30 minutes to go. Your call with the Windchasers team for {{service_interest}} is at {{booking_time}}.\nWe're getting things ready for you.`,
  'windchasers_proxe_reengagement_engaged': `Hi {{customer_name}}, you mentioned {{pain_point}} was on your mind. If that's still the case, let's talk.\nWe've been helping aspiring pilots navigate exactly that.`,
  'windchasers_proxe_reengagement_noengage': `Hi {{customer_name}}, we connected a while back but didn't get a chance to dig in.\nWant to see how a free Demo Class at Windchasers Aviation Academy could work for you?`,
  'windchasers_proxe_first_outreach': `Hi {{customer_name}}, thanks for your interest in Windchasers! We'd love to learn more about your aviation goals. When's a good time to chat?`, // NOT YET SUBMITTED TO META — placeholder text
  'windchasers_proxe_post_call_followup': `Hi {{customer_name}}, thanks for the great conversation! If you have any questions about our courses, feel free to reach out. We're here to help!`, // NOT YET SUBMITTED TO META — placeholder text
  'windchasers_proxe_followup_engaged': `Hi {{customer_name}}, we were talking about {{service_interest}} at Windchasers. Ready to take the next step?`,
  'windchasers_proxe_followup_noengage': `Hi {{customer_name}}, you reached out to us recently about {{service_interest}}. Would you like to know how we can help you get started?`,
  'windchasers_proxe_rnr': `Hi {{customer_name}}, we tried reaching you but couldn't connect. Would you like to schedule a call at a time that works for you?`, // NOT YET SUBMITTED TO META — placeholder text
};
...
async function getTemplatePreview(task, lead) {
  ...
  if (taskType === 'booking_reminder_24h' || taskType === 'reminder_24h') {
    return {
      name: 'windchasers_proxe_booking_reminder_24h',
      params: [
    ...
    return {
      name: 'windchasers_proxe_booking_reminder_30m',
      params: [
        { label: 'Name', parameter_name: 'customer_name', value: leadName },
        { label: 'Service', parameter_name: 'service_interest', value: serviceInterest },
        { label: 'Time', parameter_name: 'booking_time', value: bookingTime },
    ...
    if (engaged) {
      return {
        name: 'windchasers_proxe_reengagement_engaged',
        params: [
          { label: 'Name', parameter_name: 'customer_name', value: leadName },
    ...
    return { name: 'windchasers_proxe_reengagement_noengage', params: [{ label: 'Name', parameter_name: 'customer_name', value: leadName }] };
  } else if (taskType === 'first_outreach') {
    return { name: 'windchasers_proxe_first_outreach', params: [{ label: 'Name', parameter_name: 'customer_name', value: leadName }] };
  } else if (taskType === 'post_call_followup') {
    return { name: 'windchasers_proxe_post_call_followup', params: [{ label: 'Name', parameter_name: 'customer_name', value: leadName }] };
    ...
    const lastWasNoEngage = lastTemplate === 'windchasers_proxe_followup_noengage';
    const lastWasEngaged = lastTemplate === 'windchasers_proxe_followup_engaged';
    ...
    if (engaged) {
      ...
      if (lastWasEngaged) {
        ...
        return {
          name: 'windchasers_proxe_followup_noengage',
          params: [
            { label: 'Name', parameter_name: 'customer_name', value: leadName },
            { label: 'Service', parameter_name: 'service_interest', value: serviceInterest },
          ],
      ...
      return {
        name: 'windchasers_proxe_followup_engaged',
        params: [
          { label: 'Name', parameter_name: 'customer_name', value: leadName },
          { label: 'Service', parameter_name: 'service_interest', value: serviceInterest },
        ],
    ...
    return { name: 'windchasers_proxe_rnr', params: [{ label: 'Name', parameter_name: 'customer_name', value: leadName }] };
  }
}
...
async function generateMessage(task) {
  ...
  try {
    ...
    const historyText = (history || [])
      .reverse()
      .map(m => `${m.sender === 'customer' ? 'Lead' : 'Aria'}: ${m.content}`)
      .join('\n');
    ...
    const userPrompt = [
      ...
      '',
      'Write a single WhatsApp message for Aria to send right now. Follow your soul and the task intent guide above. Plain text only — no markdown, no bullet points.',
