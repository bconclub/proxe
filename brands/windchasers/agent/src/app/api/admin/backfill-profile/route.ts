 ...
 * have conversation history but are missing user_type / course_interest etc.
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const lead of leads || []) {
      ...
      if (onlyMissing && (brandCtx.user_type || brandCtx.course_interest)) {
      ...
      if (report.examples.length < 10) {
        report.examples.push({
          ...
          after: {
            ...
            course_interest: mergedBrandCtx.course_interest || null,
            timeline: mergedBrandCtx.timeline || null,
            education: mergedBrandCtx.education || null,
            city: mergedBrandCtx.city || null,
          },
        });
      }
