...
export const CURRENT_WEBINAR = {
  ...
  rule:
    'If a user asks about the webinar: give the date/time + 1–2 things it covers. FORMAT IT STACKED, not as one long paragraph — put the title/date-time on its own line, what it covers on the next, and the question or link on its own line, separated by blank lines (double line breaks). Before sharing a register link, if you do NOT already know whether they are a parent or a student/aspirant, ASK that first ("Are you registering as a student/aspiring pilot, or as a parent?") on its own line and share the matching link ONLY after they answer — parent → parentRegisterUrl, student/aspirant → registerUrl. If it is already clear from the conversation, skip the question and share the right link directly. Present the link as a plain full URL on its own line (the chat makes it clickable). It is FREE. Do NOT invent a Zoom link, price, or speaker names. Never use scarcity phrasing ("limited seats", "seats are limited").',
} as const;
