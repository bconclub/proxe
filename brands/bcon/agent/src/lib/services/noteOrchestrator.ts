 ...
 *   2. Call Claude Haiku to classify into a fixed set of categories
 ...
 *
 * Ported from the Windchasers shared orchestrator and adapted for BCON:
 *   - Generic B2B categories only (no aviation/pilot copy or AFFORDABILITY).
 *   - Claude calls use BCON's existing raw-fetch pattern (no token-usage helper).
 *   - activities.created_by is a text column in BCON → 'PROXe AI' (not a UUID).
 *   - Customer-facing WhatsApp sends are free-form, generic copy, and gated to
 *     the 24h customer-initiated window. No WC template names are referenced.
 */
...
For session_type (only meaningful for BOOKING_MADE / DEMO_TAKEN): "offline" if they will come IN PERSON — "visit our office", "come to the office", "in-person", "on-site meeting". "online" if it's a video/Zoom/Google-Meet/online session. null if not stated. An in-person office visit is ALWAYS "offline".
...
- BOOKING_MADE: ONLY when an actual demo/meeting/call was BOOKED/CONFIRMED for the lead to ATTEND, at a specific slot — e.g. "demo booked", "meeting scheduled for Fri 4pm", "booked his demo for Monday 3pm", "meeting set". A real appointment, not a plan to phone them. NEVER classify a plan to CALL the lead ("call back tomorrow", "follow up", "callback") as BOOKING_MADE — that is POST_CALL. If there's no actual booked demo/meeting, it is NOT a booking.
- NOT_POTENTIAL: "not potential", "not a fit", "wrong audience", "spam", "fake enquiry", "no budget", "too small" — genuinely not worth pursuing.
...
- NOT_INTERESTED: explicit disinterest — "not interested", "dead lead", "won't go ahead", "decided not to proceed". A LOST lead.
...
Example: note "wants to visit our office on 19-06-2026 around 2:30 pm" → {"category": "BOOKING_MADE", "booking_date": "19-06-2026", "booking_time": "2:30 pm", "session_type": "offline", "name": null, "send_message": null, "summary": "Booked an in-person office visit on 19 Jun, 2:30pm"}
 ...
 *
 * Uses BCON's raw-fetch pattern to the Anthropic Messages API (same as the
 * previous inline admin-notes implementation), so there is no extra dependency
 * on a token-usage helper.
 */
...
export function resolveBookingDate(dateStr: string, timeStr: string | null): Date {
  ...
  if (lower === 'today') {
    ...
    // falling back to tomorrow. Parse it here.
...
export async function classifyAndAct(input: ClassifyAndActInput): Promise<OrchestratorResult> {
  ...
  // Don't duplicate what the human already did. If the note says the team
  ...
  if (classification.category === 'BOOKING_MADE') {
    ...
    // an in-person visit is stored as session_type 'offline'.
    try {
  ...
  // BCON's activities.created_by is a text column (unlike WC's UUID column), so
  // we can store the human-readable actor 'PROXe AI' directly.
