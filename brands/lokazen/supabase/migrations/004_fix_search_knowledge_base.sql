-- Migration 004 (Lokazen): repair the search_knowledge_base RPC
--
-- Symptom (Lokazen prod logs, 2026-07-13):
--   [KnowledgeSearch] RPC returned error, falling back to ILIKE:
--     missing FROM-clause entry for table "c"
--
-- Root cause: Lokazen's DB has a stale/broken copy of the search_knowledge_base
-- function whose body references alias `c` (the knowledge_base_chunks CTE) in a
-- query that doesn't declare it in FROM. Every scout KB lookup errors and falls
-- back to a slower ILIKE scan. This re-creates the chunks table (idempotent) and
-- CREATE-OR-REPLACEs the function with the canonical, correct definition
-- (mirror of windchasers migration 030). Safe to run on a DB that already has
-- the table — nothing is dropped, only the function is replaced.
...
-- 4. RLS (auth disabled — same as other brands)
ALTER TABLE knowledge_base_chunks ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Allow all kb_chunks select" ON knowledge_base_chunks;
DROP POLICY IF EXISTS "Allow all kb_chunks insert" ON knowledge_base_chunks;
DROP POLICY IF EXISTS "Allow all kb_chunks update" ON knowledge_base_chunks;
DROP POLICY IF EXISTS "Allow all kb_chunks delete" ON knowledge_base_chunks;
...
-- 5. THE FIX — canonical search_knowledge_base (every `c` is declared in its FROM)
...
-- 6. Backfill any JSONB chunks already on knowledge_base rows into the table.
INSERT INTO knowledge_base_chunks (
  knowledge_base_id, chunk_index, content, char_start, char_end, token_estimate
)
SELECT
  kb.id,
  (chunk->>'index')::INTEGER,
...
FROM knowledge_base kb, jsonb_array_elements(kb.chunks) AS chunk
