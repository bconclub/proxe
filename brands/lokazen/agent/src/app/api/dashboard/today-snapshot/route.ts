...
export async function GET(request: Request) {
  try {
    ...
    // ── 1b) Lead type breakdown — Brand (needs space) vs Owner (lists space) ──
    const leadTypeOf = (l: any): 'Brand' | 'Owner' | null => {
      const uc = l?.unified_context || {};
      const raw = String(
        uc?.[BRAND_ID]?.user_type || uc?.[BRAND_ID]?.business_type ||
        uc?.windchasers?.user_type || uc?.windchasers?.business_type ||
        uc?.web?.user_type || uc?.whatsapp?.user_type ||
        uc?.raw_form_fields?.user_type || ''
      ).toLowerCase();
      // Lokazen audiences: brands seeking space vs property owners listing.
      if (raw.includes('owner') || raw.includes('property') || raw.includes('landlord')) return 'Owner';
      if (raw.includes('brand') || raw.includes('tenant') || raw.includes('space')) return 'Brand';
      return null;
    };
    const byType: Record<string, number> = { Brand: 0, Owner: 0 };
    for (const l of leads) {
