...
function buildSystemPrompt(
  ...
  if (capturedDetails && typeof capturedDetails === 'object') {
    const LABELS: Record<string, string> = {
      ...
      property_location_name: 'Location (from the map pin they shared)',
      property_coords: 'Coordinates', google_maps_url: 'Google Maps link',
...
function buildUserPrompt(params: {
  ...
  const dateContext = (channel === 'whatsapp' || channel === 'web')
    ? (() => {
        ...
        // brands only, so it must never be offered here (callers were seeing a
