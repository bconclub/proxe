...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  // More than one workspace means this deployment can see both brands' rows and
  // should offer an in-app switch. One (or none) means there is nothing to
  // switch to, so the control is not rendered at all.
  ...
  return (
    <div className="dashboard-layout min-h-screen" style={{ backgroundColor: 'var(--bg-primary)', minHeight: '100vh' }}>
      ...
      >
        ...
        {/* Workspace switcher — same domain, same session, the other brand's
            data. Expanded only: collapsed there is no room for a label and the
            icon alone would be a mystery. */}
          ...
          <div className="dashboard-layout-sidebar-navigation-list flex-1">
            ...
            {navigation.map((item, index) => {
              ...
              const needsDivider = brandId === 'pop'
                ...
                : DIVIDER_AFTER_INDICES.includes(index - 1)
      ...
      >
        ...
        {pathname === '/dashboard/inbox' || pathname === '/dashboard' || pathname === '/war-room' || pathname === '/dashboard/pipeline' ? (
