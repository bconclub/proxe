...
### 2026-05-03
- chore: cross-brand aesthetic sync — windchasers patterns ported to bcon, proxe, master (LeadDetailsModal session-table booking fallback + faded border, LeadsTable Booking-as-chip + tightened column widths in bcon, inbox scoreVisual helper for consistent Hot/Warm/Cold colors)
- feat(windchasers): live lead score in inbox right panel — calculate client-side from messages + context, fall back to stored value (`8ae6dffa`)
- fix(windchasers): inbox right panel "Messages" stat counts customer-sent only, not agent replies (`8ae6dffa`)
- fix(windchasers): right panel "Upcoming" booking lookup expanded to nested `unified_context.web.booking.date` path (inbound demo form path) (`8ae6dffa`)
- feat(windchasers): conversation list shows ScoreRing only on selected row, name-only on unselected per design (`fa9ff8ab`)
- fix(windchasers): score color/label scheme unified across inbox conversation list, right panel, and lead modal — 90+ Hot green / 70-89 Warm orange / 0-69 Cold blue. Previously inbox showed Warm in green (modal's Hot color) (`c8330ba4`)
- fix(windchasers): lead modal had no visible border on dark backdrop — added rgba(255,255,255,0.12) faded border + layered shadow (`c8330ba4`)
- fix(windchasers): leads table column widths rebalanced (Lead 27→18%, Contact 20→18%, Booking 16→12%, Type/Course bumped); BOOKING / TYPE / COURSE cells converted to compact chip pills (`c8330ba4`)
- fix(windchasers): lead modal Key Event empty for booked leads — session-table fallback (`web_sessions`/`whatsapp_sessions` lookup by phone or email when all_leads + unified_context have no booking) (`71c964cb`)
- fix(windchasers): broaden `isBookingIntent` to catch consultation, slot, reschedule, cancel, "when is it" patterns so existing-booking context loads (`84775d25`)
- feat(windchasers): true SSE streaming — `engine.processStream` yields tokens via `streamResponse` instead of awaiting full response, with parallel DB calls (`searchKnowledgeBase` + `checkBooking` via `Promise.all`)
- feat(windchasers): Anthropic prompt caching enabled in `claudeClient.ts` (anthropic-beta header + `cache_control: ephemeral` on system block) — ~80% input token reduction after first message
- feat(windchasers): conditional tool use — booking tools only attached when `isBookingIntent(message)` is true
- fix(windchasers-widget): SSE `done` event yielded BEFORE `generateFollowUps` so flow-override buttons render fast instead of waiting on a 1-3s LLM follow-up call
- fix(windchasers-widget): `onMessageComplete` now called outside `setMessages` updater (React drops nested state updates silently) — was the root cause of flow-override buttons not rendering
- fix(windchasers-widget): added useEffect safety net that watches `lastAiMessage.hasStreamed` and applies `pendingFlowOverrideRef.current` if the callback chain races (`227652d8`)
- chore(windchasers-widget): updated Aria welcome to "Hi, I am Aria, Your AI Aviation Counsellor."
- chore(windchasers-widget): `sanitizeAssistantText` strips em-dashes (→ `-`) and `[BUTTONS:...]` markers from streamed text
...
- **Backend / autonomous infra** lives in BCON (`brands/bcon/`): voice agent, task worker, admin notes, human handoff. These are being upstreamed to master.
- **Dashboard UI / inbox aesthetics** live in **windchasers** (`brands/windchasers/`) as of 2026-05-03: live lead score calc, unified Hot/Warm/Cold color scheme, faded modal border, leads-table chips, conversation-list polish. Synced to bcon, proxe, and master on 2026-05-03 — windchasers stays the reference going forward for these patterns.
...
- **Windchasers task worker / executor**: no agent_tasks worker is running for windchasers (Vercel-deployed, no PM2 process; `vercel.json` has no cron). `engine.scheduleFlowTasks` keeps inserting `first_outreach`, `nudge_waiting`, `push_to_book` rows but nothing drains the queue, so all Next Actions read "Now" past their scheduled time. Options: (a) Vercel cron route that ports `task-worker.js` `processPendingTasks` to a Next handler, (b) run the BCON worker against windchasers credentials on a VPS, or (c) Supabase Edge Function + `pg_cron`.
- Auto-cancel stale `agent_tasks` rows when the lead's state invalidates them (e.g. cancel `push_to_book` once the lead actually books)
- Multi-message session bookings sometimes don't sync `booking_date` from the channel session row to `all_leads`. The new modal session-table fallback hides the symptom; the underlying sync gap in `storeBooking` still needs fixing.
